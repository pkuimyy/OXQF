...
| 8 | Linux and Windows read the same vectors and produce deterministic bytes | Linux GCC/Clang and the pinned `windows-2022` image with an explicit Visual Studio 2022 generator for MSVC run the same suite. Both generate `writer-fingerprint.txt`; the dependent `release-evidence` CI job requires exact equality after normalizing only the diagnostic JSON transport line endings. | Pass |

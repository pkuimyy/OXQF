...
| 8 | Linux and Windows read the same vectors and produce deterministic bytes | Linux GCC/Clang and Windows MSVC run the same suite. Both generate `writer-fingerprint.txt`; the dependent `release-evidence` CI job requires exact equality. | Pass |

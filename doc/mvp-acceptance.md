...
| 8 | Linux and Windows read the same vectors and produce deterministic bytes | Linux GCC/Clang and an explicit Visual Studio 2022 generator for Windows MSVC run the same suite. Both generate `writer-fingerprint.txt`; the dependent `release-evidence` CI job requires exact equality after normalizing only the diagnostic JSON transport line endings. | Pass |
...
- `oxq_writer_fingerprint` regenerates the canonical OXQ semantic baseline through the public Writer and refuses any byte difference from the committed snapshot.
